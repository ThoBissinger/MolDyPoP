...
save(sprintf('%s/%s_dataset_spinwavedispersion',basedir,curmodel));
...
%% 2 Plot c vs T for a few N
figure
omega_src = "omega_0"; % Fits will be to omega_0 as computed in the time domain
omega_src = "omega_0_om0fit"; % Fits will be to omega_0 as computed in the time domain
% omega_src = "omega_1"; % Fits will be to omega_1 as computed in the time domain
% omega_src = "omega_2"; % Fits will be to omega_2 as computed in the Fourier domain
i_N_start=3;
c_map=linspecer(N_N - i_N_start + 1);
for i_N = i_N_start:N_N
    sqrtN = sqrtN_vals(i_N);
...
%         'Color',c_map(i_q,:));
    hold on;
    if omega_src == "omega_0"
        data_cur = c_spinwavespeed_omega_0_vals(i_N,:);
    elseif omega_src == "omega_0_om0fit"
        data_cur = c_spinwavespeed_omega_0_om0fit_vals(i_N,:);
    elseif omega_src == "omega_1"
        data_cur = c_spinwavespeed_omega_1_vals(i_N,:);
    elseif omega_src == "omega_2"
        data_cur = c_spinwavespeed_omega_2_vals(i_N,:);
    end
    h_fit_data(i_N) = plot(T_vals,data_cur,...
        'LineStyle', '-.', ...
        'LineWidth', 2, ...
        'HandleVisibility', 'off', ...
        'Color',c_map(i_N - i_N_start + 1,:));

    h_sim_data(i_N) = plot(T_vals,data_cur,'s',...
        'LineStyle', 'none', ...
        'Marker', markersym, 'MarkerSize', 3.8, ...
        'LineWidth', 1, ...
        'HandleVisibility', 'off', ...
        'Color',c_map(i_N - i_N_start + 1,:),...
        'MarkerFaceColor',c_map(i_N - i_N_start + 1,:),'MarkerEdgeColor','k');

    h_leg_data(i_N) = plot(NaN,NaN,...
        'LineStyle', '-.', ...
        'LineWidth', 1.3, ...
        'Marker', markersym, 'MarkerSize', 3.8, ...
        'DisplayName', dispname, ...
...
figname=sprintf('%s/%s_c_spinwavespeed_%s',basedir,curmodel,omega_src);
