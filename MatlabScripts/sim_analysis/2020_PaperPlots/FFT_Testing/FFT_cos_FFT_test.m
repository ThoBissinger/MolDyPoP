...
if ispc
    fig_base = "C:/Users/tbiss/Thesis/MatlabScripts/sim_analysis/2020_PaperPlots/FFT_Testing/plots/FFT_cos_FFT_test";
    ...
    fig_base = "/home/user/thobi/Code/1911_XYModelBasedOnMHoefler/MatlabScripts/sim_analysis/2020_PaperPlots/FFT_Testing/plots/FFT_cos_FFT_test";
...
% cos_func=@(t,omega_0,sigma) ones(size(t));
FFT_exact=@(om,omega_0,sigma) sqrt(.5 * pi*sigma^2) * ...
    ( exp(-.5*sigma^2*( om - omega_0 ).^2) + exp(-.5*sigma^2*( om + omega_0 ).^2) );
%% 1 Examples for (resolved) cosine functions
figure(1);
...
omega_vals=[3e-2,4e-2,5e-3,1e-2];
sigma_vals=[1e3,5e3,8e3,1e5];
c_map=linspecer(numel(omega_vals));
for i = 1:numel(omega_vals)
    omega=omega_vals(i);
    sigma=sigma_vals(i);
    dispname=sprintf('$\\omega_0 = %.1f \\cdot 10^{-2},\\ \\sigma = %.1f \\cdot 10^{3}$',omega*1e2,sigma/1e3);
    plot(times,cos_func(times,omega,sigma)-2.5*(i-1),...
...
ylim([-9, 3]);
xlabel('t');
ylabel('$h_{\sigma}(t)$','Interpreter','latex');
...
figname=sprintf('%s/cos_Examples',fig_base);
...
for i = 1:numel(sigma_vals)
    ...
    sigma=sigma_vals(i);
    [ft_vals,om_vals]=FT_correlation(times,cos_func(times,omega,sigma)+rand_strength*rand(size(times)), 0);
%     dispname=sprintf('$\\omega_0 = %.1f \\cdot 10^{-2}, \\sigma = %.1f \\cdot 10^{3}$',omega*1e2,sigma/1e3);
...
pos = get(gca, 'position');
dim = [pos(3)-.05 .8*pos(2) pos(3) pos(4)];
str = {omega_str,dt_str,tmax_str};
% str = {'$\omega_0 = 6 \cdot 10^{-2}$',sprintf('$\\Delta t = %.1f$',dt),'$t_{\textrm{max}} = 10^4$'};
annotation('textbox',dim,'String',str,'FitBoxToText','on',...
    'interpreter','latex');
% 
figname=sprintf('%s/cos_FFT_sigma_compare',fig_base);
fprintf('Completed figure %s\n',figname)
if(saveswitch == 1)
   exportgraphics(gcf,sprintf('%s.png',figname),'Resolution',300);
   exportgraphics(gcf,sprintf('%s.pdf',figname),'ContentType','vector');
...
for i = 1:numel(rand_strength)
    omega=omega_vals(1);
    sigma=sigma_vals(1);
    [ft_vals,om_vals]=FT_correlation(times,cos_func(times,omega,sigma)+rand_strength(i)*rand(size(times)), 0);
...
for i = 1:numel(dt_vals)
    dt=dt_vals(i);
    times=0:dt:tmax;
    [ft_vals,om_vals]=FT_correlation(times,cos_func(times,omega,sigma), 0);
    dispname=sprintf('$\\Delta t = %.1f$',dt);
    plot(om_vals,abs(real(ft_vals)),...
        'DisplayName',dispname, ...
        'Color',c_map(i,:), ...
        'LineWidth',2);
...
plot(om_vals,FFT_exact(om_vals,omega,sigma),...
        'DisplayName','exact', ...
        'Color',c_map(end,:), ...
        'LineWidth',2);
xlim([-.15,.15]);
...
pos = get(gca, 'position');
dim = [pos(3)-.05 .8*pos(2) pos(3) pos(4)];
str = {omega_str,sigma_str,tmax_str};
annotation('textbox',dim,'String',str,'FitBoxToText','on',...
    'interpreter','latex');
% 
figname=sprintf('%s/cos_FFT_dt',fig_base);
fprintf('Completed figure %s\n',figname)
if(saveswitch == 1)
   exportgraphics(gcf,sprintf('%s.png',figname),'Resolution',300);
   exportgraphics(gcf,sprintf('%s.pdf',figname),'ContentType','vector');
...
for i = 1:numel(n_points_vals)
    ...
    times=0:dt:tmax;
    [ft_vals,om_vals]=FT_correlation(times,cos_func(times,omega,sigma), n_points);
...
pos = get(gca, 'position');
dim = [pos(3)-.05, .05, pos(3), .3*pos(4)];
% str = {'$\omega_0 = 6 \cdot 10^{-2}$','$\sigma = 5\cdot 10^{3}$','$t_{\textrm{max}} = 10^4$'};
str = {omega_str,sigma_str,tmax_str};
annotation('textbox',dim,'String',str,'FitBoxToText','on',...
    'interpreter','latex');
% 
figname=sprintf('%s/cos_FFT_spline_add',fig_base);
fprintf('Completed figure %s\n',figname)
if(saveswitch == 1)
   exportgraphics(gcf,sprintf('%s.png',figname),'Resolution',300);
   exportgraphics(gcf,sprintf('%s.pdf',figname),'ContentType','vector');
