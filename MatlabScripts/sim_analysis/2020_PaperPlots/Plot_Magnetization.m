...
%         absM_var=cell2mat(data.('absM_var'));
    ...
    [T_C,T_star,T_KT,crossover_M,mag_fitfuncs] = fit_MagnetizationScaling(T_vals,absM_av,sqrtN_vals,sqrt(2));
    %% 1 Create basic plot
    figure
    hold on
...
%     set(gcf,'units','points','OuterPosition',[0 0 240 3/4*240]);
    ...
    c_map = colormap_sqrtN();
    for i_N = 1 : N_sqrtN
        hMag_dots(i_N) = line(T_vals,absM_var(i_N,:)/sqrt(runmax));
...
%     hTitle = title(curtitle);
    ...
    for i_N = 1 : N_sqrtN
        ...
        hMag_dots(i_N) = line(T_vals,absM_av(i_N,:).*((2*L_vals(i_N)).^(.5*eta_vals_FS_Mag)));
        set(hMag_dots(i_N), ...
            'LineStyle', '--', ...
...
load(mxydata_name,'absM_av');
absM_av_mxy=cell2mat(absM_av);
load(fmxydata_name,'absM_av');
absM_av_fmxy=cell2mat(absM_av);
L_vals=[9.25,18.5,37,74,148];
T_max = .4;
labels=['N = (16)^{2}', 'N = (32)^{2}', 'N = (64)^{2}', 'N = (128)^{2}', 'N = (256)^{2}'];
hold on
...
for i_T = 1:numel(T_vals)
    ...
    for i = 1:numel(fitfunc)
        fitobs{i,i_T} = fit(sqrtN_vals(:),log(absM_av(:,i_T)),fitfunc{i},'StartPoint',StartPoints{i});
