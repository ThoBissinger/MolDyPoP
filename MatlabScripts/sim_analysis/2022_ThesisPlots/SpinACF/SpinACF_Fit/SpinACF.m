clear
pathbase="/home/user/thobi/Code/1911_XYModelBasedOnMHoefler/MatlabScripts/sim_analysis/2022_ThesisPlots/SpinACF/SpinACF_Fit";
cd(pathbase)
...
if model=="mxy"
    ...
    modeldir="mxy_3.00";
    rho = sqrtN_vals(end)^2/L_vals(end)^2;
...
elseif model=="xy_s"
    ...
    sqrtN_vals = [16 32 64 128];
    L_vals = sqrtN_vals;
    T_str = [".10", ".20", ".30", ".40", ".50", ".60", ".70", ".80", ".85", ".91", ".95", "1.00", "1.10", "1.20", "1.30"];
...
savefile=sprintf("%s/%s_data",basedir,model);
save(savefile);
%%
q_cell=cell(n_N,n_T);
...
for i_N=1:n_N
    ...
    for i=1:numel(T_select)
        ...
        q_integers=intersect(find(gmperpmperp(1:numel(qbin))~=0),q_integers);
...
%     savefile=sprintf("%s/%s_data",basedir,model);
    save(savefile,...
        'rho','q_cell', 'fitob_ga_cell', 'file_cell', 'data_cell', 'ga_vals_cell', ...
        'sigma_vals', 'sigma_err_vals', 'sigma_cimin_vals', 'sigma_cimax_vals', ...
        'fitob_om_cell', 'om_1_vals_cell', 'c_vals', 'c_err_vals', 'c_cimin_vals', ...
        'c_cimax_vals', 'H_x_vals', 'H_y_vals', 'I_x_2_vals', 'I_y_2_vals', 'Helicity_vals',...
        '-append');
...
% 
% 
% figname=sprintf('%s/%s_ga_fullT_err_%sfit_%s_sqrtN_%d',basedir,model,fittype,qmode,sqrtN);
...
text(xlim_omega_inset(1)+0.01,ylim_omega_inset(2)-.002,'$c^2 / \Upsilon$',...
...
hLegend = legend('Location', 'SouthEast','interpreter','latex',...
    'NumColumns',1, 'FontName', 'cmr12','FontSize', fontsize_annotation);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
pos=get(gca,"InnerPosition");
axes("Position",[pos(1)+.08, pos(2)+.05, .45*pos(3), pos(4)/2])

for i_N = 1:n_N
    sqrtN=sqrtN_select(i_N);
    i_N_full=find(sqrtN_vals == sqrtN);
    dispname=sprintf('$N = (%d)^2$',sqrtN);
    h_plot{i_N} = plot(T_select,c_vals(i_N,:).^2./Helicity_vals(i_N,:)*rho,...
        'DisplayName',dispname,...
        'LineStyle', linestyle, ...
        'LineWidth', linewidth, ...
        'Marker', 'o',...
...
xlim(xlim_omega_inset); xwidth=diff(xlim_omega_inset);
ylim(ylim_omega_inset); ywidth=diff(ylim_omega_inset);
text(xlim_omega_inset(1)+0.08*xwidth,ylim_omega_inset(2)-.06*ywidth,'$c^2 \rho / \Upsilon$',...
        'VerticalAlignment','top','HorizontalAlignment','left',...
        'interpreter','latex',...
        'Color','k','FontSize', fontsize_annotation);
text(xlim_omega_inset(2)-0.06*xwidth,ylim_omega_inset(1)+.02*ywidth,'$T$',...
        'VerticalAlignment','bottom','HorizontalAlignment','right',...
        'interpreter','latex',...
        'Color','k','FontSize', fontsize_annotation);    
set(gca, 'FontName', 'cmr12','FontSize', fontsize_axis,...
...
figname=sprintf('%s/%s_c_plot',pathbase,model);
% figname=sprintf('%s/%s_omegagamma_SpinwaveGammaexp',basedir,model);
if(saveswitch == 1)
    fprintf('Creating figure %s\n',figname)
    exportgraphics(gcf,sprintf('%s.png',figname),'Resolution',dpi_png);
...
figname=sprintf('%s/%s_sigmagamma',pathbase,model);
% figname=sprintf('%s/%s_omegagamma_SpinwaveGammaexp',basedir,model);
if(saveswitch == 1)
    fprintf('Creating figure %s\n',figname)
    exportgraphics(gcf,sprintf('%s.png',figname),'Resolution',dpi_png);
