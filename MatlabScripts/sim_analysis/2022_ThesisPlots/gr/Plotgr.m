clear
cd /home/user/thobi/Code/1911_XYModelBasedOnMHoefler/MatlabScripts/sim_analysis/2022_ThesisPlots/gr
...
set(h_axis, 'FontSize', fontsize_axis,'TickLabelInterpreter','latex');
set([hXLabel, hYLabel], 'FontSize', fontsize_ax_labels);
set(hLegend, 'FontSize', fontsize_annotation)
...
pos=get(gca,'InnerPosition');
annotation_str = {sprintf('$N = (%d)^2$',sqrtN),'$\rho=2.99$'};
dim=[pos(1)+2*x_border pos(2)+7*y_border .15 2*sublabel_box(2)];
...
text(NW(1),NW(2),'$g(r)$',...
    ...
    'interpreter','latex',...
    'Color','black','FontSize', fontsize_ax_labels);
set(gca, 'FontSize', fontsize_axis,'TickLabelInterpreter','latex'); %'FontName', 'cmr12',
...
print(gcf, '-dtiff', sprintf('%s.tiff',figname),'-r600');
print(gcf, '-depsc', sprintf('%s.eps',figname),'-r600');
% print(gcf, '-dtiff', sprintf('%s.tiff',figname),'Resolution',300,'Compression','none');
...
% set(h_axis, 'FontName', 'cmr12','FontSize', fontsize_axis);
% set([hXLabel, hYLabel], 'FontName', 'cmr12','FontSize', fontsize_ax_labels);
set(h_axis, 'FontSize', fontsize_axis);
set([hXLabel, hYLabel], FontSize', fontsize_ax_labels);
...
pos = [pos(1) + pos(3) - inset_w - x_border,...     % Inset left edge
    pos(2) + 6*y_border,...                         % Inset bottom edge
    inset_w,...                                     % Inset width
    inset_h];                                       % Inset height
ax_inset = axes('Position',pos);
...
figname=sprintf('%s/mxy_DiffCoeff_sqrtN_%d',basedir,sqrtN);
fprintf('Creating figure %s\n',figname)
...
for i_T = 1:numel(snapshot_T_vals)
    ...
    figure
    set(gcf,'units','centimeters','InnerPosition',[0 0 .5*columnwidth_cm .5*columnwidth_cm]); 
%     curfile=sprintf('%s/%s/run_%d/output/%s.out',...
%         snapshot_basedir,snapshot_T_str{i_T},snapshot_runnr(i_T),snapshot_sampfilename);
...
%     set([hXLabel, hYLabel], 'FontName', 'cmr12','FontSize', fontsize_ax_labels);
    ...
    figname=sprintf('%s/mxy_snap_sqrtN_%d_T_%.3f',basedir,sqrtN,T);
    fprintf('Creating figure %s\n',figname)
